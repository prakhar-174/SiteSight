...
export function FinalCTA() {
  ...
  useEffect(() => {
    if (status === 'complete') {
      setTimeout(() => {
        document.getElementById('report-view-cta')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
      }, 300);
    }
  }, [status]);
  ...
  return (
    <>
      ...
      >
        <div className="rounded-[3rem] border-2 border-[var(--color-border)] bg-[var(--color-card)] px-8 py-16 md:px-20 md:py-24 text-center">
          
          <h2 className="text-4xl md:text-6xl font-black font-bricolage text-[var(--color-text-primary)] mb-12 leading-tight">
            Ready to see your real<br />SEO score?
          </h2>
